...
function preload () {
    ...
    game.load.image('dragon_image', 'dragon_atlas.png');
    // the texture atlas data (TexturePacker JSON Array format) for the dragon bones sprite
    // (loaded independently to make it easily accessible to dragonbones)
    game.load.json('dragon_atlas', 'dragon_atlas.json');
    // load the texture atlas again so that it's content is registered in the atlas frame cache
    game.load.atlas('atlas1', 'dragon_atlas.png', 'dragon_atlas.json');
    // the dragonbones skeleton data
    game.load.json('dragon', 'dragon_skeleton.json');
}

function create () {
    var dragon=addDragonBones();
    dragon.scale.x=0.5;
    dragon.scale.y=0.5;
